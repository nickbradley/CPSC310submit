...
let AppSetting: IAppSetting = {
  ...
  requestLimit: {
    ...
    minDelay: process.env.MIN_REQUEST_DELAY || 43200000//900000//21600000//43200000
  },
  cmd: {
    concurrency: process.env.WORKERS || 1,
    timeout: process.env.CMD_TIMEOUT || 10*60*1000, // milliseconds
...
function notify(submission: ISubmission, msg: string) {
  if (submission.show) {
...
function formatTestReport(testReport: any): string {
  ...
  if (fails) {
    ...
    testReport.allFailures.forEach((failedTest: any) => {
      ...
      //let stringEnd: number = testCodeName.length - (test.indexOf(".")+1 || test.indexOf("\n")+1 || test.length);
...
let manageHandler = Router({ mergeParams: true });
router.use("/manage", manageHandler);
manageHandler.get("/", (req:any, res:any) => {
...
manageHandler.post("/", (req:any, res:any) => {
